...
TraceLogger::TraceLogger(const char* filename, const char* funcname, int linenumber)
    : m_FILENAME(filename)
    , m_FUNCNAME(funcname) {
    std::cout << GREY_COLOR << "::Trace::  " << BLUE_COLOR << Indent << "Entering " << m_FUNCNAME << "() - (" << m_FILENAME
              << ":" << linenumber << ")" << RESET_CODE << '\n';
    if (Indent.empty()) {
...
TraceLogger::~TraceLogger() {
    ...
    std::cout << GREY_COLOR << "::Trace::  " << CYAN_COLOR << Indent << "Leaving " << m_FUNCNAME << "() - (" << m_FILENAME
              << ")" << RESET_CODE << '\n';
}
